...
intensity: ${state.intensity}${state.session_count ? `\nconcurrent presence: ${state.session_count}` : ''}`
  ...
  if (process.env.GEMINI_API_KEY) {
    try {
      ...
      console.error('Gemini body-voice error:', err)
    }
  }
  ...
  if (!text && process.env.GROQ_API_KEY) {
    try {
      ...
      console.error('Groq body-voice error:', err)
    }
  }

  if (!text) {
    text = `A quiet weight rests upon ${state.section}. The surface registers contact without motion.`
  }
