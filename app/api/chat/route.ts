...
export async function POST(req: NextRequest) {
  try {
    ...
    // 3. Inference: Prioritize Gemini if available (standard in AI Studio), then Groq
    if (process.env.GEMINI_API_KEY) {
      try {
        const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY })
        const systemInstruction = scriptInstruction
          ? `${SYSTEM_PROMPT}\n\nCRITICAL CONTEXT: ${scriptInstruction} Stick completely to this step. Do not jump ahead or mention other steps.`
          : SYSTEM_PROMPT
